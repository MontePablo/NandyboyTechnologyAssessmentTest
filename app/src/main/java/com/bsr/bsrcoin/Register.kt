...
import kotlinx.android.synthetic.main.fragment_view_cheque.send_recSpinner
                                ...
                                object : StringRequest(
                                    ...
                                    { s ->
                                        try {
                                            ...
                                            Log.d("TAG refcode fatch",e.localizedMessage)
                                            e.stackTrace
                                        }
                                    },
                                    { error ->
                                        progressDialog.dismiss()
                                        Log.d("TAG refcode fatch2",error.localizedMessage)
                                        error.stackTrace
                                    }
                                ){
            ...
            object  : StringRequest(
                ...
                { s ->
                    try {
                        ...
                        Log.e("Register Process", "$res")
                        val response=res.getString("message")
                        if(response.contains("success")){
                            val intent = Intent(this@Register, Login::class.java)
                            startActivity(intent)
                            finishAffinity()
                        }
                        Toast.makeText(this@Register, response, Toast.LENGTH_LONG).show()
                    } catch (e : JSONException) {
                        Log.e("Register Process1", e.localizedMessage)
                        e.stackTrace
                    }
                ...
                { error ->
                    ...
                    progressDialog.dismiss()
                    Log.e("Register Process2", error.localizedMessage)
